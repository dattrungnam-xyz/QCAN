...
public class PostDAO {
    ...
    public ArrayList<Post> getListPostByIdFler(int IdFler)
    {
        ...
        try {
            Connection connection = ConnectDB.getConnection();
            String sql = "\n" +
                    "SELECT post.IdPost,post.PostTime,post.IdUser,post.SongName,post.SongType,post.Musician,post.PostContent,post.Video, account.fullname,account.nickname,account.avatar \n" +
                    "from post, account,follow\n" +
                    " WHERE post.IdUser = account.id and post.IdUser = follow.idFled and follow.idFler =?\n" +
